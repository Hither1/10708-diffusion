...
class MultimodalImitationObjective(AbstractObjective):
    """
    ...
    def __init__(self, scenario_type_loss_weighting: Dict[str, float], weight: float = 1.0):
        ...
        """
        self._name = 'multimodal_imitation_objective'
        self._weight = weight
    ...
    def compute(self, predictions: FeaturesType, targets: TargetsType, scenarios: ScenarioListType) -> torch.Tensor:
        ...
        # Actually does not use predictions['trajectory']
        offsets = predictions['offsets']
        logits = predictions['logits']
        targets = targets['trajectory']
        kmeans = predictions['kmeans']
        target_idxs, target_offsets = kmeans.encode(targets.data.reshape(-1,48))

        # Regression loss
        pred_offsets = offsets[range(offsets.shape[0]),target_idxs]
        pred_offsets = pred_offsets.reshape(offsets.shape[0],16,3)
        target_offsets = target_offsets.reshape(offsets.shape[0],16,3)
        regression_loss = (
            self._fn_xy(pred_offsets[...,:2], target_offsets[...,:2]).mean(dim=-1) + \
            self._fn_heading(pred_offsets[...,2], target_offsets[...,2])
        ).mean()

        # Clasifier loss
        classifier_loss = F.cross_entropy(logits, target_idxs)

        return classifier_loss + regression_loss
