...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
                 ...
                 device=None, dtype=None, use_sigma=False) -> None:
    ...
    def forward(
        ...
        # TODO: this is off so we can visualize attention. don't actually use this layer for performance
        ...
        x = src
        if self.norm_first:
            sa_out, weights = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, is_causal=is_causal, sigma=sigma)
            x = x + sa_out
            x = x + self._ff_block(self.norm2(x), sigma=sigma)
        else:
            sa_out, weights = self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal, sigma=sigma)
            x = self.norm1(x + sa_out)
            x = self.norm2(x + self._ff_block(x, sigma=sigma))
    ...
    # feed forward block
    def _ff_block(self, x: Tensor, sigma: Tensor = None) -> Tensor:
        if sigma is not None:
            x = self.adaln(x, sigma)
