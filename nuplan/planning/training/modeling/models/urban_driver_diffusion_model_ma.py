...
class UrbanDriverDiffusionMAModel(UrbanDriverOpenLoopModel):
    def __init__(
        self,
    ...
    def denoise(self, ego_trajectory, sigma, scene_features, scene_feature_masks):
        ...
        if self.use_coarse_to_fine_attention:
            ...
            attn_mask = self.generate_coarse_to_fine_masks(sigma, trajectory_pos, scene_pos)
        else:
            attn_mask = None
        ...
        if self.use_coarse_to_fine_attention:
            # generate attention mask
            all_pos = torch.cat([scene_pos, trajectory_pos], dim=1)
            attn_mask = self.generate_coarse_to_fine_masks(sigma, all_pos, all_pos)
            # only affects trajectory -> scene attentions
            attn_mask[:,-16:,-16:] = False
            attn_mask[:,:-16,:-16] = False
            attn_mask[:,:-16,-16:] = False
            attn_mask[:,-16:,-17] = False # noise token
        else:
            attn_mask = None
