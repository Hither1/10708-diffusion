...
def _plot_points(
    ...
    grad = None,                # Plots gradients arrows if included
    score = None,                # Plots scores if included
    ...
    for line_idx in range(points.shape[0]):    
        traj_x, traj_y = points[line_idx,:,0], points[line_idx,:,1]

        if grad is not None:
            grad = grad.detach().cpu().numpy() * 10
            grad_x, grad_y = grad[line_idx,:,0], grad[line_idx,:,1]
            grad_x = (grad_x / pixel_size).astype(np.int32)
            grad_y = (grad_y / pixel_size).astype(np.int32)
        ...
        if grad is not None:
            for i, (point, grad) in enumerate(zip(zip(coords_y, coords_x), zip(grad_y, grad_x))):
                cv2.circle(image, point, radius=radius, color=colors[i], thickness=-1)
                end_point = (int(point[0] + grad[0]), int(point[1] + grad[1]))
                cv2.arrowedLine(image, (point[0], point[1]), end_point, color=(255,255,255), thickness=thickness)
        else:
            for i, point in enumerate(zip(coords_y, coords_x)):
                cv2.circle(image, point, radius=radius, color=colors[line_idx].tolist(), thickness=-1)

        if use_lines:
            for i, (point_1, point_2) in enumerate(zip(zip(coords_y[:-1], coords_x[:-1]), zip(coords_y[1:], coords_x[1:]))):
                cv2.line(image, point_1, point_2, color=colors[line_idx].tolist(), thickness=thickness)
