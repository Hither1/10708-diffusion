...
class FactorizedDiffusionPlanner(AbstractPlanner):
    ...
    def generate_constraints(self, features):
        ...
        return [
            ...
            make_speed_constraint(weight=10.0),

            # TODO: Goal reaching
            make_goal_constraint(self.local_goal, weight=1.0),
    ...
    def compute_planner_trajectory(self, current_input: PlannerInput) -> AbstractTrajectory:
        ...
        if self.goal is None:
            goal = []
            current = _get_lanes(self, 2)
            for _ in range(1):
                next = _get_next(self, current)
                goal.append(_get_points_of_lane(self, current))
                current = next
            
            goal = np.concatenate(goal)
            self.set_static_goal(goal)
            # self.set_static_goal(np.array([[18.0, -20.0, 0]],
            #                               [[19.0, -20.0, 0]]))
        ...
        if self.replan_counter % self.replan_freq == 0:
            ...
            self.scores.append(np.array(out['score'].cpu()))
            ...
            if self.dump_gifs:
                ...
                if self._counter == self._counter_max:
                    ...
                    np.save(f'{fname_dir}/scores_warm_start.npy', np.array(self.scores))
...
def visualize_denoising(features, trajs):
    ...
    fname_dir = '/zfsauton2/home/huangyus/nuplan-diffusion/nuplan/planning/simulation/planner/ml_planner/viz'
