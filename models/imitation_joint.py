...
class ImitationModel(pl.LightningModule):
    ...
    def create_agent_embedder(self):
        ...
        self.agents_embedder_future = Embedder_VAE((self.agent_dim + self.T * self.wp_dim) * self.num_agents, self.emb_dim * self.num_agents, expand_theta=True, layer_norm=False)
        #  if self.q_dec:
    ...
    def create_map_encoder(self):
        ...
        self.post_map_encoder = MapEncoder(
            self.map_dim,
            self.light_dim,
            self.stop_dim,
            self.emb_dim,
    ...
    def create_agent_encoder(self):
        ...
        self.map_cross_layers, self.post_map_cross_layers = nn.ModuleList(self.map_cross_layers), nn.ModuleList(self.map_cross_layers)
    ...
    def map_cross_fn(self, agents_emb, agents_masks, map_emb, map_masks, layer):
        ...
        B, H, _, _ = agents_emb.shape
        agents_emb = agents_emb.reshape(B, H, self.num_agents, self.emb_dim)
        B, H, N, d = agents_emb.shape
        agents_emb = agents_emb.reshape((B * H, N, d))
    ...
    def get_encoding(self, obs, gt_wps=None):
        if gt_wps == None:
            ...
            for _ in range(self.num_modes):
                agents_emb = torch.normal(agents_emb_mean, torch.exp(agents_emb_log_std))
            ...
            return agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks, features, None
            ...
            agents_features = torch.flatten(agents_features, start_dim=2).unsqueeze(2)
            agents_emb_mean, agents_emb_log_std = self.agents_embedder(agents_features)
            ...
            gt_wps = gt_wps.transpose(1, 2).reshape(B, 1, 1, self.num_agents * self.wp_dim * self.T)
            post_agents_emb_mean, post_agents_emb_log_std = self.agents_embedder_future(torch.cat((agents_features, gt_wps), dim=-1))
            post_agents_emb = torch.normal(post_agents_emb_mean, torch.exp(post_agents_emb_log_std))

            post_dist = Normal(post_agents_emb_mean.clone().detach(), torch.exp(post_agents_emb_log_std.clone().detach()))
            kl_loss = torch.distributions.kl.kl_divergence(dist, post_dist).mean()


            post_map_emb, post_map_masks = self.post_map_encoder(
                map_features,
                map_masks_original,
                light_features=light_features,
                light_masks=light_masks,
            ...
            for i in range(self.num_enc_layers):
                if not self.skip_temporal_attn_fn:
                    post_agents_emb = self.post_temporal_attn_fn(post_agents_emb, agents_masks.clone(), layer=self.temporal_attn_layers[i])
                post_agents_emb = self.map_cross_fn(
                    post_agents_emb,
                    agents_masks.clone(),
                    post_map_emb,
                    post_map_masks.clone(),
                    layer=self.post_map_cross_layers[i],
                )
                post_agents_emb = self.social_attn_fn(post_agents_emb, agents_masks.clone(), layer=self.post_social_attn_layers[i])

            agents_emb = self.agent_emb_ln(agents_emb)
            post_agents_emb = self.agent_emb_ln(post_agents_emb)

            return post_agents_emb, agents_masks, post_map_emb, post_map_masks, route_emb, route_masks, features, kl_loss
    ...
    def get_decoding(self, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks, features, modes=None):
        ...
        if self.q_dec:
            ...
            #  q_base_agents_emb = self.q_decoding_agents_embedder(agent_features)
            #  out_seq = q_base_agents_emb.reshape((B, 1, -1, self.num_agents, self.emb_dim))
    ...
    def forward(self, obs, modes=None):
        agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks, features, kl_loss = self.get_encoding(obs)
    ...
    def get_visualizations(self, features=None, obs=None):
        ...
        #  vehicles = obs['vehicle_features'].detach().cpu().numpy()[:, -1]
        #  vehicles_masks = obs['vehicle_masks'].detach().cpu().numpy()[:, -1]
        # TODO
        #  roads = obs['map_features'].detach().cpu().numpy()
        #  road_masks = obs['map_masks'].detach().cpu().numpy()
        #  lights = obs['light_features'].detach().cpu().numpy()[:, -1, :, :2]
        #  light_masks = obs['light_masks'].detach().cpu().numpy()[:, -1]
        #  stops = obs['stop_features'].detach().cpu().numpy()
        #  stop_masks = obs['stop_masks'].detach().cpu().numpy()
        #  routes = obs['route_features'].detach().cpu().numpy()
