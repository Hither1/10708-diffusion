...
# from src.models.imitation_vae import ImitationModel
# from src.models.imitation_joint import ImitationModel
...
class WPImitationModel(ImitationModel):
    ...
    def get_decoding(self, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks, features, modes=None):
        ...
        if self.q_dec:
            ...
            out_seq[..., :self.agent_dim] += agents_features.unsqueeze(1)
    ...
    def forward(self, obs, prefix, gt_next_wps=None, modes=None, return_wp=True, return_act=True):
        if return_wp:
            outputs, logits, features, kl_loss = super().forward(obs, prefix, gt_next_wps, modes=modes)

            pred_next_wps = self.get_pred_next_wps(obs, outputs)

        if not return_act:
            return pred_next_wps, logits, features, kl_loss
        else:
            # TODO modes and time
    ...
    def run_step(self, batch, prefix, optimizer_idx=None):
        ...
        if (optimizer_idx is None) or (optimizer_idx == 0):
            pred_wps, logits, features, kl_loss = self.forward(obs, prefix, gt_wps, return_act=False)
            ...
            if self.use_wandb:
                ...
                self.log(f'{prefix}/kl_loss', kl_loss.item(), batch_size=batch_size)
                self.log(f'{prefix}/loss', wps_loss.detach() + 100 * kl_loss.item(), batch_size=batch_size)
        ...
        if (optimizer_idx is None) or (optimizer_idx == 1):
            ...
            pred_actions = self.forward(obs, prefix, gt_wps, return_wp=False)
        ...
        if optimizer_idx is None:
            self.prev_preds = [pred_wps, pred_actions, logits]
            return wps_loss + 100 * kl_loss, act_loss, pred_wps, features
        elif optimizer_idx == 0:
            return wps_loss + 100 * kl_loss, pred_wps, features
