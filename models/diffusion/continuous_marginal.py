...
class Model_Cond_Diffusion(nn.Module):
    ...
    def loss_on_batch(self, gt_trajectory, traj_mask, agents_emb, agents_mask, map_emb, map_mask, route_emb, route_masks):
        B, T, N, D = gt_trajectory.shape
        # gt_trajectory = self.pca_whitener.transform_features(gt_trajectory).reshape(B, T, N, self.k)
        ...
        # predictions = self.pca_whitener.untransform_features(predictions)
        loss_weight = (sigma**2 + self.sigma_data**2) / (sigma * self.sigma_data)**2


        return loss_weight, noisy_trajectory, predictions # self.pca_whitener.untransform_features(noisy_trajectory)

    def denoise_with_preconditioning(self, trajectory, trajectory_masks, sigma, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks):
        """
    ...
    def denoise(self, trajectories, trajectory_masks, sigma, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks):
        """
        ...
        """
        # Diffusion
        trajectory_features = self.trajectory_encoder(trajectories) # [:, None]
        ...
        out = self.nn_model(trajectory_features, trajectory_masks, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks)
    ...
    def sample(self, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks, return_y_trace=True, extract_embedding=False):
        batch_size = agents_emb.shape[0]
        num_agents = min(agents_emb.shape[2], self.num_agents)
        y_shape = (batch_size, 1, num_agents, self.k)

        # sample initial noise, y_0 ~ N(0, 1),
        ego_gt_trajectory = torch.randn(y_shape).to(agents_emb.device)
        ...
        # Sampling / inference
        ego_trajectory = torch.randn(ego_gt_trajectory.shape, device=agents_emb.device) * self.sigma(self.ts[0])
        ...
        for i in range(self.n_T):
            t = self.ts[i]
            t_next = self.ts[i+1]
            sigma = self.sigma(t)[None, None, None, None].repeat(batch_size*predictions_per_sample, 1, 1, 1)
            sigma_next = self.sigma(t_next)[None, None, None, None].repeat(batch_size*predictions_per_sample, 1, 1, 1)

            # Increase noise temporarily.
            # gamma = min(self.sigma_churn / self.n_T, np.sqrt(2) - 1) if self.sigma_min <= self.sigma(t) <= self.sigma_max else 0
            # t_hat = torch.as_tensor(sigma + gamma * sigma, device=agents_emb.device)
            # ego_trajectory = ego_trajectory + (t_hat ** 2 - sigma ** 2).sqrt() * self.sigma_noise * torch.randn(ego_trajectory.shape, device=agents_emb.device)
            ...
            with torch.enable_grad():
                f = lambda x: (-1 / sigma) * self.denoise_with_preconditioning(x, None, sigma, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks)
            ...
            if return_y_trace and (i % (self.n_T // 5) == 0 or i == self.n_T or (self.n_T - i) < 3):
                y_i_store.append(self.pca_whitener.untransform_features(ego_trajectory).detach().cpu().reshape(batch_size, predictions_per_sample, num_agents, self.y_dim))
                
        ego_trajectory = self.pca_whitener.untransform_features(ego_trajectory)
        ego_trajectory = ego_trajectory.reshape(batch_size, predictions_per_sample, num_agents, self.y_dim)
...
class Model_mlp(nn.Module):
    def __init__(self, n_T, y_dim, emb_dim, num_heads, 
        ...
        self.k = 10  # sometimes overwrite, eg for discretised, mean/variance, mixture density models
        self.q_dec = None
        self.p_dec = None
    ...
    def forward(self, traj_emb, traj_mask, agents_emb, agents_masks, map_emb, map_masks, route_emb, route_masks):
