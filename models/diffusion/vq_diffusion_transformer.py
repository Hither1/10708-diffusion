...
from src.models.diffusion.transformer_utils import Text2ImageTransformer
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        num_embed=32,
        content_seq_len=20, # Number of tokens, should be similar to number of agents
        diffusion_step=100,
        ...
        self.quantize = VQVAE(carla_maps_path=carla_maps_path, 
                              T=T)
    ...
    def predict_start(self, traj_emb, x_t, wps_mask, agents_emb, agents_masks, map_emb, map_masks, t):          # p(x0|xt)
        # x_t = log_onehot_to_index(log_x_t)
        if self.amp == True:
            with autocast():
                out = self.transformer(traj_emb, wps_mask, agents_emb, agents_masks, map_emb, map_masks, t)
        else:
            out = self.transformer(traj_emb, wps_mask, agents_emb, agents_masks, map_emb, map_masks, t)
        
        assert out.size(0) == traj_emb.size(0)
        assert out.size(1) == self.num_classes-1
        assert out.size()[2:] == x_t.size()[1:]
        log_pred = F.log_softmax(out.double(), dim=1).float()
        batch_size = traj_emb.size()[0]
        if self.zero_vector is None or self.zero_vector.shape[0] != batch_size:
            self.zero_vector = torch.zeros(batch_size, 1, self.content_seq_len) - 70
        log_pred = torch.cat((log_pred, self.zero_vector.to(log_pred.device)), dim=1)
    ...
    def _train_loss(self, traj_emb, traj_encoding_indices, wps_mask, agents_emb, agents_masks, map_emb, map_masks, is_train=True):              # get the KL loss
        b, device = traj_emb.size(0), traj_emb.device

        assert self.loss_type == 'vb_stochastic'
        x_start = traj_encoding_indices
        t, pt = self.sample_time(b, device, 'importance')
        log_x_start = index_to_log_onehot(x_start, self.num_classes).squeeze()
        log_xt = self.q_sample(log_x_start=log_x_start, t=t)
        xt = log_onehot_to_index(log_xt)

        ############### go to p_theta function ###############
        log_x0_recon = self.predict_start(traj_emb, xt, wps_mask.permute(0, 2, 1)[..., 0], 
                                            agents_emb, agents_masks, map_emb, map_masks, t=t) # P_theta(x0|xt)         
        ...
        return log_model_prob, vb_loss, x0_recon.clone().detach()
    ...
    def forward(
        ...
        if kwargs.get('autocast') == True:
            self.amp = True
        traj_emb, traj_encoding_indices = self.quantize.get_discretized(gt_wps)

        batch_size = agents_emb.shape[0]
        device = agents_emb.device

        # 1) get embeddding for condition and content     prepare input
        agent_emb = agents_emb.type_as(agents_emb)
        map_emb = map_emb.type_as(map_emb)
        ...
        if is_train == True:
            log_model_prob, loss, pred_index = self._train_loss(traj_emb, traj_encoding_indices, wps_mask, agent_emb, agents_masks, map_emb, map_masks)
            loss = loss.sum() / (traj_emb.size()[0])
    ...
    def sample_fast(
        ...
        with torch.no_grad():
            ...
            for diffusion_index in diffusion_list:
                ...
                t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                traj_emb = self.quantize.get_quantized_emb(log_onehot_to_index(log_z))
                traj_masks = torch.ones(traj_emb.shape[0], self.content_seq_len).to(traj_emb.device)

                log_x_recon = self.predict_start(traj_emb, log_onehot_to_index(log_z), traj_masks, agents_emb, agents_masks, map_emb, map_masks, t)
