...
from src.models.imitation_model import ImitationModel
...
from src.envs.carla.autopilots.navigation import controller
...
class WPImitationModel(ImitationModel):
    def __init__(
        ...
        if self.pid_type == 'slow':
            ...
            args_longitudinal_dict = {
                ...
                'K_I': 0.05,
                'dt': dt}
            self.longitudinal_controller = controller.PIDLongitudinalController(
            ...
            #  args_lateral_dict = {
                ...
                #  'K_I': 0.0,
                #  'dt': dt}
            #  self.lateral_controller = controller.PIDLateralController(
                    #  #  self._vehicle,
                    #  None,
            ...
            #  self.fast_turn_controller = PIDController(K_P=1.5, K_I=0., K_D=0.3, n=20)
            self.fast_turn_controller = PIDController(K_P=1.5, K_I=0., K_D=0., n=20)
        else:
    ...
    def forward(self, obs, gt_next_wps=None, modes=None, return_wp=True, return_act=True):
        if return_wp:
            outputs, logits, features = super().forward(obs, modes=modes)
    ...
    def run_step(self, batch, prefix, optimizer_idx=None):
        ...
        if (optimizer_idx is None) or (optimizer_idx == 0):
            ...
            #  loss, logit_loss, wps_errors, act_errors, time_wps_errors = self._compute_loss(pred_wps, pred_actions, logits, gt_wps, gt_actions, wps_mask, actions_mask)
        ...
        if (optimizer_idx is None) or (optimizer_idx == 1):
            ...
            gt_actions = gt_actions[:, self.H-1].to(self.device)
            actions_mask = actions_mask[:, self.H-1].to(self.device)

            pred_actions = self.forward(obs, gt_wps, return_wp=False)

            act_loss, act_errors = self._compute_action_loss(pred_actions, gt_actions, wps_mask, actions_mask)
            act_error = act_errors.sum(dim=-1)
            #  self.inverse_dynamics_model.update_std(act_errors, tau=self.std_update_tau)

            if self.use_wandb:
                batch_size = gt_wps.shape[0]
                self.log(f'{prefix}/act_loss', act_loss.detach(), batch_size=batch_size)
                self.log(f'{prefix}/act_error', act_error.detach(), batch_size=batch_size)

                for i in range(len(act_errors)):
                    self.log(f'{prefix}/act_error[{i}]', act_errors[i].detach(), batch_size=batch_size)

        if optimizer_idx is None:
            self.prev_preds = [pred_wps, pred_actions, logits]
            return wps_loss, act_loss, pred_wps, features
    ...
    def validation_step(self, batch, batch_idx):
        wp_loss, act_loss, pred_actions, features = self.run_step(batch, prefix='val')
        ...
        return wp_loss, act_loss, pred_actions
    ...
    def control_pid(self, waypoints, curr_speed):
        ...
        if self.pid_type == 'slow':
            ...
            delta = np.clip(desired_speed - speed, -0.25, 0.25)
            throttle = self.speed_controller.step(delta)
            throttle = np.clip(throttle, -1.0, 0.75)
            #  if ((throttle < 0.0) or (desired_speed < 0.1)):
            if (throttle < 0.0):
                gas = -1.0
                brake = True
            else:
            ...
            gas = self.longitudinal_controller.pid_control(3.6 * desired_speed, 3.6 * speed, enable_brake=True)
            #  brake = gas < 0.
    ...
    #  def control_pid_fast(self, waypoints, curr_speed):
        #  wps = waypoints[:, :2].data.cpu().numpy() * self.max_token_distance
        #  speed = curr_speed.data.cpu().numpy() * self.max_token_distance

        #  desired_speed = np.maximum(speed + waypoints[0, 3].data.cpu().numpy() * self.max_token_distance, 0.0)
        ...
        #  #  steer = self.lateral_controller.pid_control(aim, numpy=True)
    ...
    def get_visualizations(self, features=None, preds=None, obs=None):
        ...
        pred_image = np.zeros((num_samples, 2, self.im_shape[0], self.im_shape[1], 3), dtype=np.uint8)
